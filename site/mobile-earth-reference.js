(() => {
  ...
  const VERSION='0.3.2R4F4R1-single-earth-thermal';
  ...
  let raf=0,lastDraw=0,lastStatic=0,lastSize='',staticDirty=true,motionUntil=0;
  ...
  function ringsFromGeometry(g){
    if(!g)return [];
    const out=[];
    if(g.type==='Polygon'){if(Array.isArray(g.coordinates?.[0]))out.push(g.coordinates[0]);}
    else if(g.type==='MultiPolygon'){for(const p of g.coordinates||[]){if(Array.isArray(p?.[0]))out.push(p[0]);}}
  ...
  function buildDots(rings){
    ...
    for(const ring of rings){
      ...
      if(maxLon-minLon>220)continue;minLat=Math.max(-82,minLat);maxLat=Math.min(84,maxLat);
      const lonStart=Math.ceil(minLon/DOT_STEP)*DOT_STEP,latStart=Math.ceil(minLat/DOT_STEP)*DOT_STEP;
      for(let lat=latStart;lat<=maxLat;lat+=DOT_STEP){for(let lon=lonStart;lon<=maxLon;lon+=DOT_STEP){if(!pointInRing(lon,lat,ring))continue;const seed=deterministic(lon,lat),jl=(seed-.5)*.42,jb=(deterministic(lat,lon)-.5)*.30,key=`${Math.round((lon+jl)*10)}:${Math.round((lat+jb)*10)}`;if(seen.has(key))continue;seen.add(key);dots.push([lon+jl,lat+jb,seed]);if(dots.length>=MAX_DOTS)return dots;}}
    }
    return dots;
  }
  ...
  async function loadGeometry(){
    try{const [land,countries]=await Promise.all([getJson(LAND_URL),getJson(COUNTRIES_URL)]);geometry.landRings=flattenFeatures(land);geometry.countryRings=flattenFeatures(countries);geometry.dots=buildDots(geometry.landRings);if(geometry.landRings.length<20||geometry.dots.length<1500)throw new Error('GEO_INCOMPLETE');geometry.ready=true;geometry.error='';staticDirty=true;app.classList.add('loky-earth-reference-ready');if(state.active)startLoop();}
    catch(error){geometry.error=String(error?.message||error||'GEO_FAIL');geometry.ready=false;app.classList.remove('loky-earth-reference-ready');console.warn('[LOKY Earth] Natural Earth no disponible',geometry.error);}
  }
  ...
  function syncCanvas(){
    ...
    if(key!==lastSize){canvas.width=w;canvas.height=h;cacheCanvas.width=w;cacheCanvas.height=h;lastSize=key;staticDirty=true;}
    return {w,h};
  }
  function project(lon,lat,radius,cx,cy){return seismic.project(lon,lat,radius,cx,cy);}
  function drawRing(target,ring,radius,cx,cy,stroke,width,alpha=1){let pen=false,visible=false;target.beginPath();for(const pnt of ring){const p=project(pnt[0],pnt[1],radius,cx,cy);if(!p){pen=false;continue;}if(!pen){target.moveTo(p.x,p.y);pen=true;}else target.lineTo(p.x,p.y);visible=true;}if(!visible)return;target.save();target.strokeStyle=stroke;target.globalAlpha=alpha;target.lineWidth=width;target.lineJoin='round';target.lineCap='round';target.stroke();target.restore();}

  function drawGrid(target,radius,cx,cy,w){
    const minor=Math.max(.42,w/1550),major=Math.max(.68,w/1120);
    for(let lat=-60;lat<=60;lat+=20){const ring=[];for(let lon=-180;lon<=180;lon+=4)ring.push([lon,lat]);drawRing(target,ring,radius,cx,cy,lat%40===0?'rgba(55,145,220,.13)':'rgba(49,123,194,.065)',lat%40===0?major:minor);}
    for(let lon=-180;lon<180;lon+=20){const ring=[];for(let lat=-86;lat<=86;lat+=4)ring.push([lon,lat]);drawRing(target,ring,radius,cx,cy,lon%40===0?'rgba(55,145,220,.12)':'rgba(49,123,194,.06)',lon%40===0?major:minor);}
  }
  function drawLand(target,radius,cx,cy,w){
    target.save();const baseSize=Math.max(.75,w/1080);target.shadowColor='rgba(65,195,255,.26)';target.shadowBlur=Math.max(1.5,w/700);
    for(const dot of geometry.dots){const p=project(dot[0],dot[1],radius,cx,cy);if(!p)continue;const s=baseSize*(.78+p.z*.82)*(dot[2]>.76?1.30:1);target.globalAlpha=.24+.66*p.z;target.fillStyle=dot[2]>.70?'rgba(118,222,255,.98)':'rgba(60,168,242,.92)';target.fillRect(p.x-s*.5,p.y-s*.5,s,s);}
    target.restore();target.globalAlpha=1;
    const glow=Math.max(2.1,w/430),fine=Math.max(.78,w/1180);
    for(const ring of geometry.landRings){drawRing(target,ring,radius,cx,cy,'rgba(39,144,235,.085)',glow);drawRing(target,ring,radius,cx,cy,'rgba(112,220,255,.58)',fine);}
    const border=Math.max(.4,w/1600);for(const ring of geometry.countryRings)drawRing(target,ring,radius,cx,cy,'rgba(77,171,229,.11)',border,.9);
  }
  function rebuildStatic(now,w,h){
    lastStatic=now;staticDirty=false;cacheCtx.clearRect(0,0,w,h);
    ...
    cacheCtx.save();cacheCtx.beginPath();cacheCtx.arc(cx,cy,radius,0,Math.PI*2);cacheCtx.clip();drawGrid(cacheCtx,radius,cx,cy,w);drawLand(cacheCtx,radius,cx,cy,w);const shine=cacheCtx.createRadialGradient(cx-radius*.42,cy-radius*.44,0,cx-radius*.28,cy-radius*.30,radius*.76);shine.addColorStop(0,'rgba(99,216,255,.12)');shine.addColorStop(.44,'rgba(35,143,220,.035)');shine.addColorStop(1,'rgba(0,0,0,0)');cacheCtx.beginPath();cacheCtx.arc(cx,cy,radius,0,Math.PI*2);cacheCtx.fillStyle=shine;cacheCtx.fill();cacheCtx.restore();
  ...
  function render(now){
    ...
    if(staticDirty&&now-lastStatic>=34)rebuildStatic(now,w,h);
    const cx=w/2,cy=h*.525,radius=Math.min(w,h)*.382*state.globeZoom;
  ...
  function frame(now){
    ...
    const moving=now<motionUntil,fps=moving?ACTIVE_FPS:IDLE_FPS;
    if(now-lastDraw>=1000/fps){lastDraw=now;render(now);}
    raf=requestAnimationFrame(frame);
  }
  function startLoop(){if(!state.active||!geometry.ready||raf)return;lastDraw=0;raf=requestAnimationFrame(frame);}
  function stopLoop(){if(raf)cancelAnimationFrame(raf);raf=0;pointers.clear();dragPointer=null;tapStart=null;tapMoved=false;}
  function markViewMotion(){staticDirty=true;motionUntil=performance.now()+420;startLoop();}
  ...
  canvas.addEventListener('pointerdown',event=>{if(!state.active)return;pointers.set(event.pointerId,{x:event.clientX,y:event.clientY});try{canvas.setPointerCapture(event.pointerId)}catch{}if(pointers.size===1){dragPointer=event.pointerId;tapStart={x:event.clientX,y:event.clientY,id:event.pointerId};tapMoved=false;}else if(pointers.size===2){pinchDistance=distance()||1;pinchZoom=state.globeZoom;dragPointer=null;tapMoved=true;}motionUntil=performance.now()+420;});
  ...
  const endPointer=event=>{const wasTap=tapStart?.id===event.pointerId&&!tapMoved&&pointers.size===1;pointers.delete(event.pointerId);if(wasTap)pickQuake(event.clientX,event.clientY);if(pointers.size===1){dragPointer=[...pointers.keys()][0];}else if(!pointers.size)dragPointer=null;if(tapStart?.id===event.pointerId)tapStart=null;};
  canvas.addEventListener('pointerup',endPointer);canvas.addEventListener('pointercancel',event=>{pointers.delete(event.pointerId);dragPointer=null;tapStart=null;tapMoved=false;});
  ...
  window.addEventListener('loky:seismic-mode',event=>{if(event.detail?.active){staticDirty=true;startLoop();}else stopLoop();});
  window.addEventListener('loky:seismic-view',()=>{markViewMotion();});
  window.addEventListener('loky:seismic-data',()=>{motionUntil=performance.now()+300;startLoop();});
  window.addEventListener('resize',()=>{lastSize='';staticDirty=true;if(state.active)startLoop();},{passive:true});
  window.addEventListener('pagehide',stopLoop,{once:true});
  loadGeometry();

  window.LOKY_PC4_EARTH_REFERENCE={version:VERSION,source:{naturalEarthCommit:NE_COMMIT,land:LAND_URL,countries:COUNTRIES_URL},geometry,reload:loadGeometry,stats:()=>({dots:geometry.dots.length,landRings:geometry.landRings.length,countryRings:geometry.countryRings.length,active:state.active,raf:Boolean(raf),fpsIdle:IDLE_FPS,fpsMoving:ACTIVE_FPS,dprCap:DPR_CAP})};
})();