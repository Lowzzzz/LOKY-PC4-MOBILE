...
assert.equal(api.version,'0.3.2R4F12R2-pronunciation-cards');
...
const card=api.extractTeachingCard('TRADUCCIÓN: ¿Cómo estás? PRONUNCIACIÓN: jáu ar YÚ');
assert(card,'teaching card must parse');
assert.equal(card.meaning,'¿Cómo estás?');
assert.equal(card.pronunciation,'jáu ar YÚ');

const cleaned=api.cleanTutorDisplay('Texto viejo que no importa. TRADUCCIÓN: hola PRONUNCIACIÓN: jeló');
assert(!cleaned.includes('TRADUCCIÓN'));
assert(!cleaned.includes('PRONUNCIACIÓN'));
...
assert(index.includes('<script src="./mobile-language-tutor.js?v=0.3.2r4f12r2"></script>'));
assert(index.indexOf('mobile-actions.js?v=0.3.2r4f11r6') < index.indexOf('mobile-language-tutor.js?v=0.3.2r4f12r2'));
