...
(async()=>{
  ...
  assert.equal(api.version,'0.3.2R4F12R7-fast-assist-3d-shell');
  ...
  assert.equal(api.cleanUserDisplay('Hola. Hola.'),'Hola.');
  assert.equal(api.cleanUserDisplay('Yes Yes'),'Yes');
  assert.equal(api.cleanTutorDisplay('Old one. Old one. Useful question?'),'Old one. Useful question?');
  assert.equal(api.cleanTutorDisplay('One. Two. Three. Four?'),'Three. Four?');
  const deduped=api.dedupeSentences(['Hello.','Hello.','How are you?']);
  assert.deepEqual(JSON.parse(JSON.stringify(deduped)),['Hello.','How are you?']);
  ...
  assert(src.includes('selected.length<2'));

  assert(!src.includes('loky-language-dialog'));
  assert(!src.includes('loky-language-status'));
  assert(!src.includes('data-lang-tutor'));
  assert(!src.includes('data-lang-user'));
  assert(src.includes('width:92%;height:86%'));
  assert(src.includes('object-position:center center'));
  assert(src.includes("live.append(stateBadge,floating,tools,progress)"));
  assert(src.includes('const AUTO_ASSIST_SETTLE_MS=90'));
  assert(src.includes('const AUTO_ASSIST_END_SETTLE_MS=180'));
  assert(src.includes('function latestCompletedTutorPhrase('));
  assert(!src.includes("clearFloatingAssist();\n        scheduleAutoAssist(tutorText);"));
  assert(src.includes("const AVATAR_3D_URL='https://novgwydgcvlboujnmygq.supabase.co/storage/v1/object/public/loky-public-assets/language/avatar-r4f12r7-v1.glb'"));
  assert(src.includes("const THREE_VERSION='0.180.0'"));
  assert(src.includes('new THREE.WebGLRenderer'));
  assert(src.includes("for(const name of ['AvatarRoot','Pelvis','Chest','Head','Jaw','Eye_L','Eye_R','Lid_L','Lid_R'])"));
  assert(src.includes("for(const keyName of ['Eye_L','Eye_R'])"));
  assert(src.includes("for(const keyName of ['Lid_L','Lid_R'])"));
  assert(src.includes('const jaw=nodes.Jaw'));
  assert(src.includes('transcriptSpeechPulse(t)'));
  assert(src.includes("fallbackImg.classList.add('is-3d-loaded')"));
  assert(src.includes('function disposeAvatar3D('));
  assert(src.includes('initAvatar3D(hero,avatar).catch(()=>{})'));
  assert(src.includes('https://cdn.jsdelivr.net/npm/three@'));
