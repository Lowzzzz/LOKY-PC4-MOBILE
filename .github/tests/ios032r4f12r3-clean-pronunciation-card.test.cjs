...
assert.equal(api.version,'0.3.2R4F12R3-clean-pronunciation-card');
...
const messy='Welcome to class. What would you like to drink? TRADUCCIÓN: ¿Qué te gustaría beber? PRONUNCIACIÓN: guat wud yú laik tu DRINK? Imagine we are at a café.';
const card=api.extractTeachingCard(messy);
assert(card,'teaching card must parse');
assert.equal(card.target,'What would you like to drink?');
assert.equal(card.meaning,'¿Qué te gustaría beber?');
assert.equal(card.pronunciation,'guat wud yú laik tu DRINK?');

const preserved=api.stripTeachingPayloads('Hello there. TRADUCCIÓN: Hola. PRONUNCIACIÓN: jeló. What is your name?');
assert(preserved.includes('Hello there.'));
assert(preserved.includes('What is your name?'));
assert(!preserved.includes('TRADUCCIÓN'));
assert(!preserved.includes('PRONUNCIACIÓN'));

const latest=api.latestTutorPhrase('Welcome. How are you today? What would you like to drink?');
assert.equal(latest,'What would you like to drink?');

const cleaned=api.cleanTutorDisplay('Old sentence. Another one. What is your name?');
assert(cleaned.endsWith('What is your name?'));
...
assert(index.includes('<script src="./mobile-language-tutor.js?v=0.3.2r4f12r3"></script>'));
assert(index.indexOf('mobile-actions.js?v=0.3.2r4f11r6') < index.indexOf('mobile-language-tutor.js?v=0.3.2r4f12r3'));
...
assert(src.includes('TRADUCCIÓN + PRONUNCIACIÓN'));
assert(src.includes('SE PRONUNCIA'));
assert(src.includes('ESCUCHAR Y REPETIR'));
assert(src.includes('data-lang-teaching-card'));
assert(src.includes('data-state="speaking"'));
