...
// Supabase deployment version: 18
// EZBR SHA256: 958b95125f23dd4ce8023a56745bb98d6ee2dcdf1da7bab46e92cb2c2a0ca81a
// Captured before local-first Bing query ordering.
...
async function localContactSearch(query:string,scope:any){
  ...
  add(`${scope.entity} ${scope.location} teléfono`);
  ...
  const merged:Array<{title:string,uri:string,snippet:string}>=[];
  const seen=new Set<string>();
  const localPhoneKeys=()=>{
    const keys=new Set<string>();
    for(const item of merged.filter(result=>locationResultMatch(result,scope))){
      for(const phone of extractPhones(String(item.title||'')+" "+String(item.snippet||'')))keys.add(phone.key);
    }
    return keys;
  };
  ...
  for(const variant of variants){
    ...
    if(localPhoneKeys().size>=2)break;
  }

  if(localPhoneKeys().size<2){
    try{
      const bing=await bingSearch(query);
      for(const item of bing){
        if(!locationResultMatch(item,scope)||seen.has(item.uri))continue;
        seen.add(item.uri);
        merged.push(item);
      }
    }catch{}
...
Deno.serve(async (req: Request) => {
  ...
  try{
    ...
    if(scope){
      provider='hybrid-local-strict-v3';
